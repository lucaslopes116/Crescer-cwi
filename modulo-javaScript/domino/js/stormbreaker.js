...
class TestAssertion {
	...
	isTrue(obtained) {
		if (obtained !== true) {
			throw new TestAssertionFailedError(
				`expected: true, obtained: ${obtained}`
			)
		}
	}
	...
	isFalse(obtained) {
		if (obtained !== false) {
			throw new TestAssertionFailedError(
				`expected: false, obtained: ${obtained}`
			)
		}
	}
	...
	throws(errorClass, methodHandler) {
		try {
			...
			if (error === 'test_failed') {
				throw new TestAssertionFailedError(
					`method not raise ${errorClass.name} error.`
				)
			}
	...
	throwsWithMessage(errorClass, errorMessage, methodHandler) {
		...
		if (errorThrowed.message !== errorMessage) {
			throw new Error(
				`expected message: "${errorMessage}", obtained message: "${
					errorThrowed.message
				}"`
			)
		}
	}
